...
def cal_shap(model, x,shapley_point, M, sp_size, MC, y_predicted):  
    #model is pre-trained CNN model, x for input image
    #M is matrix size (number of shapley values)
    #MC is monte carlo times, sp_size for super pixel size
    #shapley_point is which point we are calculating shapley value
    #sp_size is superpixel size
    ...
    for i in range(M[0]*M[1]):
        ...
        if(binom_M[i]<=MC):  #all
            ...
            for j in range(MC):
                ...
                s2=np.array(s2)
                if(s2!=[]):
                    idx1=np.array(s2[:,0],np.int32)
                    idx2=np.array(s2[:,1],np.int32)
                    ##print("s2:",s2)
                    if(np.ndim(V_plus)==4):
                        V_minus[j,idx1,idx2,:] = x[idx1,idx2,:]
                    else:
